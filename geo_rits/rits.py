...
class RITS(Model):
    ...
    def build(self, input_shape):
        self.rnn_cell = tf.keras.layers.LSTM(units=self.hid_dim, return_state=True, dropout=0.2)
    ...
    def call(self, values, masks, deltas):
        ...
        longitude_loss = tf.constant([[1, 0.5] for _ in range(len(values))], dtype=tf.float32)
        ...
        for t in range(self.sequence_length):
            ...
            err = ((tf.abs(x - x_hat) * m) / (tf.reduce_sum(m) + 1e-6)) + ((tf.square(x - x_hat) * m) / (tf.reduce_sum(m) + 1e-6))
            ...
            err = ((tf.abs(x - z_hat) * m) / (tf.reduce_sum(m) + 1e-6)) + ((tf.square(x - z_hat) * m) / (tf.reduce_sum(m) + 1e-6))
            err = err * longitude_loss 
            custom_loss += tf.reduce_sum(err, axis=1)
            ...
            err = ((tf.abs(x - c_hat) * m) / (tf.reduce_sum(m) + 1e-6)) + ((tf.square(x - c_hat) * m) / (tf.reduce_sum(m) + 1e-6))
            err = err * longitude_loss 
            custom_loss += tf.reduce_sum(err, axis=1)
